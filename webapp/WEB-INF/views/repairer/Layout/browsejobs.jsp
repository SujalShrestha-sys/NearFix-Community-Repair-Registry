...
<head>
  ...
  <title>NearFix – Browse Jobs</title>
...
<body class="bg-gray-50 flex h-screen overflow-hidden">
  ...
  <aside class="w-64 bg-white border-r border-gray-200 flex flex-col h-screen fixed left-0 top-0 z-10">
    ...
    <nav class="flex-1 px-3 py-4 flex flex-col gap-1">
      ...
      <a href="browsejobs.jsp" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-semibold text-[#1a6b3c] bg-[#e6f4ec] hover:scale-[1.02] active:scale-[0.98] transition-all duration-200">
  ...
  <div class="ml-64 flex-1 flex flex-col min-h-screen">
    <header class="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-6 sticky top-0 z-10">
      <h1 class="text-lg font-bold text-gray-900">Browse Jobs</h1>
    ...
    <main class="flex-1 p-6 overflow-y-auto">
      <div class="mb-5">
        <h2 class="text-2xl font-extrabold text-gray-900">Browse Open Jobs</h2>
        <p class="text-sm text-gray-500 mt-0.5">Find repair requests near you</p>
      </div>

      <!-- Filter Bar -->
      <div class="flex items-center gap-3 mb-5">
        <div class="flex-1 flex items-center gap-2 border border-gray-200 rounded-xl px-4 py-2.5 bg-white hover:border-[#1a6b3c]/30 hover:shadow-sm transition-all duration-300">
          <i class="fa-solid fa-magnifying-glass text-gray-400 text-sm"></i>
          <input type="text" placeholder="Search jobs..." class="text-sm text-gray-500 outline-none bg-transparent w-full"/>
        </div>
        <div class="relative hover:scale-[1.02] transition-transform duration-200">
          <select class="appearance-none border border-gray-200 rounded-xl px-4 py-2.5 pr-8 text-sm text-gray-700 bg-white cursor-pointer outline-none hover:border-[#1a6b3c]/30 shadow-sm transition-all">
            <option>All Categories</option>
            <option>Electronics</option>
            <option>Furniture</option>
            <option>Clothing</option>
          </select>
          <i class="fa-solid fa-chevron-down text-gray-500 text-xs absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
        </div>
        <div class="relative hover:scale-[1.02] transition-transform duration-200">
          <select class="appearance-none border border-gray-200 rounded-xl px-4 py-2.5 pr-8 text-sm text-gray-700 bg-white cursor-pointer outline-none hover:border-[#1a6b3c]/30 shadow-sm transition-all">
      ...
      <div class="space-y-3">
        ...
        <!-- Job 1 -->
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 hover:scale-[1.01] hover:shadow-md transition-all duration-300 cursor-pointer group">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-bold text-gray-900">Samsung Galaxy S22 screen</h3>
              </div>
              <div class="flex items-center gap-2 mb-3">
                <span class="bg-blue-50 text-blue-600 text-[11px] font-semibold px-2.5 py-0.5 rounded-full">Pending</span>
                <span class="bg-yellow-100 text-yellow-700 text-[11px] font-semibold px-2.5 py-0.5 rounded-full">Urgent</span>
                <span class="text-xs text-gray-500">Electronics · #1044</span>
              </div>
              <p class="text-sm text-gray-600 mb-4">Samsung S22 display cracked. Still functional but needs replacement screen. Thamel area.</p>
              <div class="flex items-center gap-3 text-xs text-gray-500">
                <span class="flex items-center gap-1"><i class="fa-solid fa-location-dot text-red-400"></i> Thamel</span>
                <span class="bg-yellow-100 text-yellow-700 text-[11px] font-semibold px-2.5 py-0.5 rounded-full">Urgent</span>
                <span>Posted 1h ago</span>
              </div>
            </div>
            <div class="flex flex-col items-end gap-3 ml-4">
              <button class="text-gray-300 hover:text-red-400 hover:scale-110 active:scale-90 transition-all duration-200"><i class="fa-regular fa-heart text-lg"></i></button>
        ...
        <!-- Job 2 -->
        <div class="bg-white rounded-2xl border border-[#1a6b3c]/50 shadow-sm p-5 hover:scale-[1.01] hover:shadow-md transition-all duration-300 cursor-pointer group">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-bold text-gray-900">Sony Bluetooth Speaker</h3>
              </div>
              <div class="flex items-center gap-2 mb-3">
                <span class="bg-blue-50 text-blue-600 text-[11px] font-semibold px-2.5 py-0.5 rounded-full">Pending</span>
                <span class="text-xs text-gray-500">Electronics · #1043</span>
              </div>
              <p class="text-sm text-gray-600 mb-4">Sony speaker won't turn on. Was working fine then stopped. Charging port may be damaged.</p>
              <div class="flex items-center gap-3 text-xs text-gray-500">
                <span class="flex items-center gap-1"><i class="fa-solid fa-location-dot text-red-400"></i> Boudha</span>
                <span class="bg-gray-100 text-gray-500 text-[11px] font-semibold px-2.5 py-0.5 rounded-full">Normal</span>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 hover:scale-[1.01] hover:shadow-md transition-all duration-300 cursor-pointer group">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-bold text-gray-900">Dining chair joint broken</h3>
              </div>
              <div class="flex items-center gap-2 mb-3">
                <span class="bg-blue-50 text-blue-600 text-[11px] font-semibold px-2.5 py-0.5 rounded-full">Pending</span>
                <span class="text-xs text-gray-500">Furniture · #1045</span>
              </div>
              <p class="text-sm text-gray-600 mb-1">One dining chair has a loose leg joint. Makes noise and slightly unstable. Needs proper gluing.</p>
            </div>
            <div class="flex flex-col items-end gap-3 ml-4">
              <button class="text-gray-300 hover:text-red-400 hover:scale-110 active:scale-90 transition-all duration-200"><i class="fa-regular fa-heart text-lg"></i></button>
            </div>
